...
Microsoft Teams의 핵심을 사내용으로 다시 만든 웹앱입니다. 프론트는 빌드 도구 없는 정적 파일이라 GitHub에 올려 Netlify에 연결하면 배포됩니다. 계정·데이터는 Supabase, 알림은 웹 푸시가 맡습니다. PWA라서 휴대폰에서 홈 화면에 추가하면 앱처럼 씁니다.

문서 편집은 두 가지 중 골라서 씁니다.

| 방식 | 비용 | 필요한 것 | 비고 |
|---|---|---|---|
| **ONLYOFFICE Docs** | 별도 VM 비용 발생 | 문서 서버 VM 1대 | 진짜 .docx/.xlsx/.pptx, 앱 안에서 그대로 편집 |
| **구글 문서 연동** | **완전 무료** | Google Cloud 프로젝트(무료) + 각자 구글 계정 | 서버 불필요, 편집은 새 탭에서 구글 화면으로 열림 |

VM을 두기 부담스럽다면 아래 ③' 을 보세요.
...
## ③' 구글 문서 연동 (완전 무료, VM 없이)

ONLYOFFICE 대신 **구글 Docs·Sheets·Slides를 그대로 빌려 쓰는** 방식입니다. 회사 로그인 계정(Supabase)은 그대로 두고, 문서 편집만 각자 구글 계정을 "연동"합니다. 실시간 공동편집·저장·기록은 전부 구글이 처리하므로 **문서 서버도, Netlify Function도 추가로 필요 없습니다.**

1. [Google Cloud Console](https://console.cloud.google.com) 에서 프로젝트 생성(무료).
2. **API 및 서비스 → 라이브러리** → "Google Drive API" 사용 설정.
3. **API 및 서비스 → OAuth 동의 화면**:
   - 회사가 **Google Workspace**를 쓴다면 사용자 유형을 **내부(Internal)** 로 설정하세요. 그러면 구글의 앱 심사 없이 바로 씁니다.
   - 개인 Gmail을 섞어 쓴다면 **외부(External)** 로 설정합니다. `drive.file` scope 하나만 요청하므로("이 앱으로 만들거나 연 파일"에만 접근) 보통 심사 없이 테스트 사용자 등록만으로 충분합니다.
4. **사용자 인증 정보 → OAuth 클라이언트 ID 만들기** → 유형은 **웹 애플리케이션**. **승인된 자바스크립트 원본**에 배포 주소를 등록합니다(예: `https://teamhub.netlify.app`). 리디렉션 URI는 필요 없습니다.
5. 발급된 **클라이언트 ID**를 `config.js`에 넣습니다.
...
6. **SQL Editor**에서 `supabase/schema.sql`을 다시 실행합니다(8번 섹션이 `google_email`, `files.provider` 등 필요한 컬럼을 추가합니다. 여러 번 실행해도 안전합니다).

동작 흐름:
```
프로필 → "구글 계정 연동" → 브라우저에서 구글 동의 창 → 접근 토큰만 그 자리에서 발급
                                                        (refresh token 없음, 서버 저장 없음)
Word/Excel/PPT 새로 만들기
  └─ Drive API로 빈 구글 문서 생성 → 현재 채널 멤버 중 구글 연동된 사람에게 편집 권한 공유
  └─ files 테이블엔 "누구 것, 어느 채널 것"이라는 참조만 저장 (provider='google')
문서 열기 → 새 탭에서 구글 편집 화면 → 커서·댓글·자동 저장은 전부 구글 UI
```

**알아둘 것:**
- 새로 채널에 들어온 사람, 또는 파일을 만든 뒤에 구글 계정을 연동한 사람은 자동으로 권한이 생기지 않습니다. 파일 목록의 **"권한 나누기"** 버튼을 (이미 접근 권한이 있는 누군가가) 눌러서 다시 나눠줘야 합니다.
- Google Workspace가 아니라 각자 개인 Gmail이면 사람 수만큼 이메일 공유가 필요합니다(도메인 전체 공유 불가).
- 문서 실체는 구글 드라이브에 저장됩니다. 사내 문서를 외부 서비스에 두는 것에 대해 회사 방침을 먼저 확인하세요.
- ONLYOFFICE와 동시에 켜둘 수 있습니다. `DOC_PROVIDER`를 비워두면 `GOOGLE_CLIENT_ID`가 있을 때 자동으로 구글을 쓰고, 이미 만든 파일은 `provider` 값에 따라 각자 맞는 편집기로 열립니다.
...
- [ ] Word 문서 생성 → 두 브라우저에서 동시 편집 → 커서 두 개 → 저장 후 **v2**로 오르는가 (ONLYOFFICE 사용 시)
- [ ] 프로필 → 구글 계정 연동 → Word 새로 만들기 → 새 탭에서 구글 문서가 열리는가 (구글 연동 사용 시)
...
- **구글 문서 연동을 쓰면**, 새로 채널에 들어온 사람에게 권한이 자동으로 가지 않습니다(③' 참고). 문서 실체도 구글 드라이브에 있어 사내 문서 보관 정책과 별개로 검토가 필요합니다.
...
assets/js/docs.js        문서 편집기 라우터 (onlyoffice ↔ google)
assets/js/gauth.js       구글 계정 연동 (OAuth 토큰 클라이언트, 서버 불필요)
assets/js/gdocs.js       구글 Docs/Sheets/Slides 생성·공유·열기
